...
<html lang="en">
  ...
  <body class="bgimage">
	...
	<% ArrayList<Path> paths=(ArrayList<Path>)request.getAttribute("paths");%>
      <div class="my-3 p-3 bg-trans rounded box-shadow">
        <h6 class=" border-gray pb-2 mb-0">Paths of <%=model %> </h6>
        </div>
        <% for(Path path:paths){%>
        <div class="media bg-trans text-muted pt-3  rounded p-3 ">
          <img data-src="holder.js/32x32?theme=thumb&bg=007bff&fg=007bff&size=1" alt="" class="mr-2 rounded" src="images/im.svg">
           ...
           <font color="black">
            <strong class="d-block "><%=path.getDate_path() %></strong>
           <strong>Path Id : </strong> <%=path.getId() %><br>
            </font>
          </p>
        <a class="btn btn-outline-secondary" href="/test/path?idPath=<%=path.getId()%>">Show Path</a>
        </div>
        <br>
        <% }%>
